...
!$acc end kernels
  ...
  if(ising.le.0) then
    ...
    tst=ts
#ifdef ACC
!$acc kernels present(aat,aaa,tt,ta,sm,g,lab)
...
!$omp target teams distribute parallel do reduction(+:tst)
    ...
    do intg=1,numint
      ...
      l=lab(3,intg)
      n=lab(4,intg)
      tst=tst+g(intg)*(sm(k,i)*sm(n,l)-aat(n,i)*aaa(l,k)-tt(n,i)*ta(l,k) &
          -aaa(n,i)*aat(l,k)-ta(n,i)*tt(l,k)-aat(l,i)*aaa(n,k)-tt(l,i)*ta(n,k) &
...
!$acc end kernels
  ...
  return
end subroutine gronor_gntwo_mlab
...
#ifdef ACC
!$acc kernels present(aaal,aatl,ttl,tatl,tt,aat,aaa,ta,sml) copyin(ibl)
#endif
#ifdef OMPTGT
#ifdef OMP5
!$omp target teams loop collapse(2)
...
!$acc loop vector(32)
        ...
        do ibl=1,nb0
          etotb=etotb+g(intg)*prefac(ibl)*(sml(ibl,k,i)*sml(ibl,n,l) &
              -aatl(ibl,n,i)*aaal(ibl,l,k)-ttl(ibl,n,i)*tatl(ibl,l,k) &
              -aaal(ibl,n,i)*aatl(ibl,l,k)-tatl(ibl,n,i)*ttl(ibl,l,k) &
              -aatl(ibl,l,i)*aaal(ibl,n,k)-ttl(ibl,l,i)*tatl(ibl,n,k) &
              -aaal(ibl,l,i)*aatl(ibl,n,k)-tatl(ibl,l,i)*ttl(ibl,n,k))
        enddo
      enddo
#ifdef OMPTGT
#ifdef OMP5
...
!$acc loop vector(32)
        ...
        do ibl=nhc-nb1+1,nhc
          etotb=etotb+g(intg)*prefac(ibl)*(sml(ibl,k,i)*(diagl(ibl,l)*csdiagl(ibl,n) &
              +diagl(ibl,n)*csdiagl(ibl,l)+bdiagl(ibl,l)*bsdiagl(ibl,n) &
              +bdiagl(ibl,n)*bsdiagl(ibl,l))+(diagl(ibl,i)*csdiagl(ibl,k) &
              +diagl(ibl,k)*csdiagl(ibl,i)+bdiagl(ibl,i)*bsdiagl(ibl,k) &
              +bdiagl(ibl,k)*bsdiagl(ibl,i))*sml(ibl,n,l) &
              -csdiagl(ibl,i)*(aatl(ibl,n,k)*diagl(ibl,l) &
              +aatl(ibl,l,k)*diagl(ibl,n))-bsdiagl(ibl,i)*(ttl(ibl,n,k)*bdiagl(ibl,l) &
              +ttl(ibl,l,k)*bdiagl(ibl,n))-csdiagl(ibl,l)*(aaal(ibl,n,i)*diagl(ibl,k) &
              +aaal(ibl,n,k)*diagl(ibl,i))-bsdiagl(ibl,l)*(tatl(ibl,n,i)*bdiagl(ibl,k) &
              +tatl(ibl,n,k)*bdiagl(ibl,i))-csdiagl(ibl,k)*(aatl(ibl,n,i)*diagl(ibl,l) &
              +aatl(ibl,l,i)*diagl(ibl,n))-bsdiagl(ibl,k)*(ttl(ibl,n,i)*bdiagl(ibl,l) &
              +ttl(ibl,l,i)*bdiagl(ibl,n))-csdiagl(ibl,n)*(aaal(ibl,l,i)*diagl(ibl,k) &
              +aaal(ibl,l,k)*diagl(ibl,i))-bsdiagl(ibl,n)*(tatl(ibl,l,i)*bdiagl(ibl,k) &
...
600 format(' Calculating two electron matrix element',/)
    ...
    if(ising.eq.0) then
      ...
      !!_OMPTGT_($omp target teams loop)
      !        do i=1,nbas
      !          diagl(ibl,i)=diag(i)
      !          bdiagl(ibl,i)=bdiag(i)
      !          bsdiagl(ibl,i)=bsdiag(i)
...
#endif
#ifdef OMPTGT
!$omp target update from(ta,aaa,aat,tt)
#endif
    do k=1,nbas
      do i=1,nbas
        sml(ibl,i,k)=aaa(k,i)+aaa(i,k)+ta(k,i)+ta(i,k)
...
#endif
#ifdef OMPTGT
!$omp target update to(tatl,aaal,aatl,ttl,sml)
#endif
    etotb=0.0d0
...
!$omp end target data
  ...
  return
end subroutine gronor_gntwo_batch_mlab
...
#endif
#ifdef ACC
!$acc kernels present(lab,g,sm0,aat0,aaa0,tt0,ta0,prefac0)
#endif
#ifdef OMPTGT
#ifdef OMP5
!$omp target teams loop reduction(+:etotb) private(i,k,l,n)
...
!$acc loop vector(32)
      ...
      do ibl=1,nb0
        etotb=etotb+g(intg)*prefac0(ibl)*(sm0(k,i,ibl)*sm0(n,l,ibl) &
            -aat0(n,i,ibl)*aaa0(l,k,ibl)-tt0(n,i,ibl)*ta0(l,k,ibl) &
            -aaa0(n,i,ibl)*aat0(l,k,ibl)-ta0(n,i,ibl)*tt0(l,k,ibl) &
...
!$acc loop vector(32)
      ...
      do ibl=1,nb1
        etotb=etotb+g(intg)*prefac1(ibl)*(sm1(k,i,ibl)*(diag1(l,ibl)*csdiag1(n,ibl) &
            +diag1(n,ibl)*csdiag1(l,ibl)+bdiag1(l,ibl)*bsdiag1(n,ibl)+bdiag1(n,ibl)*bsdiag1(l,ibl))&
            +(diag1(i,ibl)*csdiag1(k,ibl)+diag1(k,ibl)*csdiag1(i,ibl)+bdiag1(i,ibl)*bsdiag1(k,ibl) &
            +bdiag1(k,ibl)*bsdiag1(i,ibl))*sm1(n,l,ibl)-csdiag1(i,ibl)*(aat1(n,k,ibl)*diag1(l,ibl) &
...
!$omp end target data
  ...
  return
end subroutine gronor_gntwo_batch2_mlab
...
!$omp end parallel do
  ...
  return
end subroutine gronor_gntwo_omp_mlab
      ...
      do intg=1,numint
        ...
        do ibl=nhc-nb1+1,nhc
          etotb=etotb+g(intg)*prefac(ibl)*(sml(ibl,k,i)*(diagl(ibl,l)*csdiagl(ibl,n) &
              ...
              +diagl(ibl,k)*csdiagl(ibl,i)+bdiagl(ibl,i)*bsdiagl(ibl,k) &
              +bdiagl(ibl,k)*bsdiagl(ibl,i))*sml(ibl,n,l) &
              -csdiagl(ibl,i)*(aatl(ibl,n,k)*diagl(ibl,l)+aatl(ibl,l,k)*diagl(ibl,n)) &
              -bsdiagl(ibl,i)*(ttl(ibl,n,k)*bdiagl(ibl,l)+ttl(ibl,l,k)*bdiagl(ibl,n)) &
              -csdiagl(ibl,l)*(aaal(ibl,n,i)*diagl(ibl,k)+aaal(ibl,n,k)*diagl(ibl,i)) &
              -bsdiagl(ibl,l)*(tatl(ibl,n,i)*bdiagl(ibl,k)+tatl(ibl,n,k)*bdiagl(ibl,i)) &
              -csdiagl(ibl,k)*(aatl(ibl,n,i)*diagl(ibl,l)+aatl(ibl,l,i)*diagl(ibl,n)) &
              -bsdiagl(ibl,k)*(ttl(ibl,n,i)*bdiagl(ibl,l)+ttl(ibl,l,i)*bdiagl(ibl,n)) &
              -csdiagl(ibl,n)*(aaal(ibl,l,i)*diagl(ibl,k)+aaal(ibl,l,k)*diagl(ibl,i)) &
              -bsdiagl(ibl,n)*(tatl(ibl,l,i)*bdiagl(ibl,k)+tatl(ibl,l,k)*bdiagl(ibl,i)))
        enddo
      enddo
#ifdef OMP
!$omp end parallel do
...
!$omp do reduction(+:etotb,etemp) private(intg,jj,i,k,l,n,ibl)
        ...
        do jj=ii,kl
          ...
          etemp=0.0d0
          do ibl=1,nb0
            etemp=etemp+prefac(ibl)*(sml(ibl,k,i)*sml(ibl,n,l)-aatl(ibl,n,i)*aaal(ibl,l,k) &
                -ttl(ibl,n,i)*tatl(ibl,l,k)-aaal(ibl,n,i)*aatl(ibl,l,k) &
                -tatl(ibl,n,i)*ttl(ibl,l,k)-aatl(ibl,l,i)*aaal(ibl,n,k) &
                -ttl(ibl,l,i)*tatl(ibl,n,k)-aaal(ibl,l,i)*aatl(ibl,n,k) &
...
!$omp& private(intg,i,k,l,n,ibl,etemp)
      ...
      do ii=intndx,jntndx
        do jj=ii,kl
          ...
          etemp=0.0d0
          do ibl=1,nb0
            etemp=etemp+prefac(ibl)*(sml(ibl,k,i)*sml(ibl,n,l) &
                -aatl(ibl,n,i)*aaal(ibl,l,k)-ttl(ibl,n,i)*tatl(ibl,l,k) &
                -aaal(ibl,n,i)*aatl(ibl,l,k)-tatl(ibl,n,i)*ttl(ibl,l,k) &
                -aatl(ibl,l,i)*aaal(ibl,n,k)-ttl(ibl,l,i)*tatl(ibl,n,k) &
                -aaal(ibl,l,i)*aatl(ibl,n,k)-tatl(ibl,l,i)*ttl(ibl,n,k))
          enddo
          do ibl=nhc-nb1+1,nhc
            etemp=etemp+prefac(ibl)*(sml(ibl,k,i)*(diagl(ibl,l)*csdiagl(ibl,n) &
                +diagl(ibl,n)*csdiagl(ibl,l)+bdiagl(ibl,l)*bsdiagl(ibl,n) &
                +bdiagl(ibl,n)*bsdiagl(ibl,l))+(diagl(ibl,i)*csdiagl(ibl,k) &
                +diagl(ibl,k)*csdiagl(ibl,i)+bdiagl(ibl,i)*bsdiagl(ibl,k) &
                +bdiagl(ibl,k)*bsdiagl(ibl,i))*sml(ibl,n,l) &
...
!$omp end parallel do
  ...
  return
end subroutine gronor_gntwo_omp_batch_mlab
      ...
      do intg=1,numint
        ...
        do ibl=nhc-nb1+1,nhc
          etotb=etotb+g(intg)*prefac(ibl)* &
              (sml(ibl,k,i)*(diagl(ibl,l)*csdiagl(ibl,n)+diagl(ibl,n)*csdiagl(ibl,l) &
              +bdiagl(ibl,l)*bsdiagl(ibl,n)+bdiagl(ibl,n)*bsdiagl(ibl,l)) &
              +(diagl(ibl,i)*csdiagl(ibl,k)+diagl(ibl,k)*csdiagl(ibl,i) &
              +bdiagl(ibl,i)*bsdiagl(ibl,k)+bdiagl(ibl,k)*bsdiagl(ibl,i))*sml(ibl,n,l) &
              -csdiagl(ibl,i)*(aatl(ibl,n,k)*diagl(ibl,l)+aatl(ibl,l,k)*diagl(ibl,n)) &
              -bsdiagl(ibl,i)*(ttl(ibl,n,k)*bdiagl(ibl,l)+ttl(ibl,l,k)*bdiagl(ibl,n)) &
              -csdiagl(ibl,l)*(aaal(ibl,n,i)*diagl(ibl,k)+aaal(ibl,n,k)*diagl(ibl,i)) &
              -bsdiagl(ibl,l)*(tatl(ibl,n,i)*bdiagl(ibl,k)+tatl(ibl,n,k)*bdiagl(ibl,i)) &
...
!$omp end parallel do
  ...
  return
end subroutine gronor_gntwo_omp_batch2_mlab
