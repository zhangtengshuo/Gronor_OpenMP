...
character(len=80),dimension(2) :: runfile
character(len=80),dimension(2) :: root
...
program grotate
  ...
  integer                   :: i,j,fragment,iState,iFrag
  ...
  ! Read RUNFILE information once

  call NameRun('RUNFILE')
  print*,'Opened RUNFILE'
  ...
  write(runfile(2),'(a,a)') trim(root(1)),'.runfil'
  write(xyzfile(2),'(a,a)') trim(root(1)),'.xyz'

  do iFrag=2,nFrags

    write(runfile(1),'(a,a)') trim(root(ifrag)),'.runfil'
    write(xyzfile(1),'(a,a)') trim(root(iFrag)),'.xyz'

    do iState=1,nStates

      call grotate_get_coordinates

      write(*,'(a,i2,a,i2,a,a)') 'State ',istate,' of ',nStates,': ',trim(state(iState))

      write(orbfile(1),'(a,a,a,a)') trim(root(2)),'_',trim(state(iState)),'.orb'
      write(orbfile(2),'(a,a,a,a)') trim(root(1)),'_',trim(state(iState)),'.orb'

      if ( inporb ) then
        call grotate_readvec
...
data keyword /'FRAG','TARG','NOIN','STAT' /
...
!        write(xyzfile(2),'(a,a)') trim(root(2)),'.xyz'
      case(2)
        call locate('TARG')
        read(*,*) t1(1),t1(2)
        read(*,*) t2(1),t2(2)
...
subroutine grotate_get_coordinates
use grotate_files_data
implicit none
...
!open(35,file = 'INPORB')
print*,'Opening:',trim(orbfile(2))
open(35,file=trim(orbfile(2)))
...
!open(12,file = 'ROTORB')
write(*,'(a,a)') 'Write ',trim(orbfile(1))
open(12,file=trim(orbfile(1)))
