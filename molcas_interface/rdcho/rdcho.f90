...
!      use Cholesky
      ...
      integer                    :: nBas,nDel,nOrb,nFrozen,numCho(8)
...
#include "WrkSpc.fh"
      ...
      call Get_iArray('NumCho',numCho,1)
...
!$omp parallel private(nthreads,mythread,ipq,i,irs,iOffset,j,i1,i2,iVec) shared(integral,npq,nTransCho)
      ...
      do while(numint.gt.0)
        ...
        if(nBuf.ge.10) then
          if(mod(iBuf,int(nBuf/10)).eq.0 ) then
            write(*,'(A,I8,A,I8)') 'Writing buffer ',iBuf,' of ',nBuf
          endif
        endif
      enddo
        write (*,'(A,I4,A)') 'Writing the last buffer with ',j,' integrals'
      deallocate(integral)
...
! Last (incomplete) buffer, skip it in the rare case that the number of integrals is a multiple of the buffer size
      if ( i .ne. 0) then
        write (*,'(A,I4,A)') 'Writing the last buffer with ',i,' integrals'
!        do j = 1, i
!           write(*,'(i8,F25.15)')j,integral(j)
!        end do
       call ddafile(luTra,1,integral,bufLength,iad50)
