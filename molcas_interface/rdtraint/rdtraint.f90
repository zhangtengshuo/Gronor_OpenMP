...
real (kind=8)                     :: potNuc,scal,almostZero,percentage
...
if (iRC.ne.0) write(6,*) 'Something went wrong reading the overlap integrals from ONEINT'
iCounter = 1
do j = 1, nBas
  do k = 1, j
...
do iComponent = 1,3
  ...
  if (iRC.ne.0) write(6,*) 'Something went wrong reading the dipole integrals from ONEINT'
  iCounter = 0
  do j = 1, nBas
    do k = 1, j
      iCounter = iCounter + 1
...
do iComponent = 1,6
  ...
  if (iRC.ne.0) write(6,*) 'Something went wrong reading the quadrupole integrals from ONEINT'
  iCounter = 0
  do j = 1, nBas
    do k = 1, j
      iCounter = iCounter + 1
...
do j = 1, nOrb
  do k = 1, j
    ...
    if ( print_level .ge. 10 ) write(*,'(i6,F22.14)') iCounter,sMO(iCounter)
  end do
end do 
deallocate(dummy)
write(*,*) 
...
do iComponent = 1, 3
  ...
  do j = 1, nOrb
    do k = 1, j
      iCounter = iCounter + 1
      do l = 1, nBas
        dipole(iCounter,iComponent) = dipole(iCounter,iComponent) + vec(j,l) * aux(k,l)
      end do
    end do
  end do
end do
if ( print_level .ge. 10 ) then
  write(*,'(2a)') 'Dipole integrals transformed to the common basis'
  do i = 1, nOrbtt
...
do iComponent = 1, 6
  ...
  do j = 1, nOrb
    do k = 1, j
      iCounter = iCounter + 1
      do l = 1, nBas
        quadrupole(iCounter,iComponent) = quadrupole(iCounter,iComponent) + vec(j,l) * aux(k,l)
      end do
    end do
  end do
end do
if ( print_level .ge. 10 ) then
  write(*,'(2a)') 'Quadrupole integrals transformed to the common basis'
  do i = 1, nOrbtt
    write(*,'(i4,6F14.8)') i,(quadrupole(i,iComponent),iComponent=1,6)
  end do
end if    
...
if (vectit(40:45).eq.'tau_MO') then
  title1 = 'Integrals in common MO basis ('//vectit(40:57)//')'
else
...
do iFile = 1, nFiles
  ...
  write(filename_two_el(iFile),'(3a,i3.3,a)')trim(Project),trim(combas),'_',iFile,'.two'
...
if ( nFiles .gt. 1 ) then
  write(*,*)' Number of records on last file     :',nRecs_onFile(nFiles)
end if
write(*,*)' Number of integrals on last record :',onLastFile - (nRecs_onFile(nFiles)-1) * nTraBuf
if ( write_labels ) then
...
! Dump the integrals on the 2-el file
    if ( write_labels ) then
      ...
      write(luTwo_GronOR,rec=recNumber2)(traBuf(n),n=1,nInts),nInts,ilast
    end if
  end do
  close(luTwo_GronOR)
end do
write(*,'(A,ES8.1,A,I10)')'Number of integrals smaller than ',almostZero,' :  ',nonZeroInt
...
do j = 2, nBas
  if ( BasfnLbl(j)(1:6) .ne. BasfnLbl(j-1)(1:6) ) iAtom = iAtom + 1   ! next atom
...
iCounter = 1
lAO = 's  p  d  f  g  h  i'
write(15,'(6x,A,10x,2A,5x,A)')'Atom','x             y             z',   &
         '          Znuc',lAO
do iAtom = 1, nAtoms
  write(15,201)iAtom,AtomLbl(iAtom),(coord(k),k=iCounter,               &
         iCounter+2),zNuc(iAtom),(nCntr(iAtom,l)/(2*(l-1)+1),l=1,7)
  iCounter = iCounter + 3
end do
...
subroutine rdtraint_read_input(Project,print_level,write_labels,nTraRec,almostZero,combas)
implicit none
