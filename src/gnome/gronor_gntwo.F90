...
!$acc kernels present(aat,aaa,tt,ta,sm) async(1)
...
!$acc kernels present(aat,aaa,tt,ta,sm,g,lab,ndx) copyin(kl,intndx,jntndx) async(1)
    do ii=intndx,jntndx
      do jj=ii,kl
        intg=ndx(ii)+jj
        i=lab(1,ii)
...
!$acc kernels present(diag,bdiag,bsdiag,csdiag) async(1)
    do k=1,nbas
...
!$acc& present(diag,bdiag,bsdiag,csdiag) copyin(kl,intndx,jntndx) async(1)
    do ii=intndx,jntndx
      do jj=ii,kl
        intg=ndx(ii)+jj
        i=lab(1,ii)
...
!$acc update host(aat,aaa,tt,ta,sm,g,lab,ndx,diag,bdiag,bsdiag,csdiag) async(1)
    do ii=intndx,jntndx
      do jj=ii,kl
        intg=ndx(ii)+jj
        i=lab(1,ii)
...
!$acc kernels present(aaa0,aat0,tt0,ta0,tt,aat,aaa,ta,sm0) copyin(nb0) async(1)
      do k=1,nbas
        do i=1,nbas
...
!     ASSEMBLE ARRAYS ON GPU
!$acc kernels present(aaa1,aat1,tt1,ta1,tt,aat,aaa,ta,sm1) copyin(nb1) async(1)
      do k=1,nbas
        do i=1,nbas
...
!$acc kernels present(diag,bdiag,bsdiag,csdiag) present(diag1,bdiag1,bsdiag1,csdiag1) copyin(nb1) async(1)
      do i=1,nbas
        diag1(nb1,i)=diag(i)
...
!$acc data copyin(prefac0)
    kl=nbas*(nbas+1)/2
!$acc kernels present(lab,ndx,g,sm0,aat0,aaa0,tt0,ta0,prefac0) async(1)
    do ibl=1,nb0
      do ii=intndx,jntndx
        do jj=ii,kl
          intg=ndx(ii)+jj
...
!$acc kernels present(lab,ndx,g,sm1,aat1,aaa1,tt1,ta1,prefac1) present(diag1,bdiag1,bsdiag1,csdiag1) async(1)
    do ii=intndx,jntndx
      do jj=ii,kl
        intg=ndx(ii)+jj
        i=lab(1,ii)
