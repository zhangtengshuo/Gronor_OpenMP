...
if(idbg.ge.20) then
  do idet=1,ndets1
    call dbg_write_scalar('makebasestate','coef',coef(idet),step=idet)
    call dbg_log_msg('makebasestate',trim(occ(idet)),step=idet)
  enddo
endif
#endif
...
! create the new intermediate occupations and coefficients
      ...
      if(ms1+ms2.eq.target_spin) then
        do idet=first1,last1
          do jdet=first2,last2
            prod=coef1(idet)*coef2(jdet)
            if(abs(prod).ge.tau_CI) then
              dnorm=clebsch_gordon((spin1-1)/two,(spin2-1)/two,ms1/two,ms2/two, &
  ...
  if(idbg.ge.20) then
    do idet=1,newdets
      call dbg_write_scalar('makebasestate','coefmebf',coefmebf(idet),step=idet)
      call dbg_log_msg('makebasestate',trim(occmebf(idet)),step=idet)
    enddo
  endif
#endif
...
!     allocated in gronor_generate_microstates
    ...
    if(idbg.ge.20) then
      do idet=1,micro_dets1
        call dbg_write_scalar('makebasestate','coef',coef1(idet),step=idet)
        call dbg_log_msg('makebasestate',trim(occ1(idet)),step=idet)
      enddo
    endif
#endif
...
#ifdef DEBUG_HDF5
  if(idbg.ge.20) then
    call dbg_write_scalar('makebasestate','maxcoef',maxcoef)
  endif
#endif
  allocate(occ_num(nactb(iMEBF)))
  do idet=1,newdets
...
#ifdef DEBUG_HDF5
    if(idbg.ge.20) then
      call dbg_write_scalar('makebasestate','civb',civb(idet,iMEBF),step=idet)
      call dbg_log_msg('makebasestate',trim(dumstr),step=idet)
    endif
#endif
  enddo
