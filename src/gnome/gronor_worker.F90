...
!$omp&        ntcl,ntop,nclose,nopen,nelec,nact,ninact)
  ...
  allocate(a(nelecs,nelecs),           source=0.0d0)
  allocate(ta(mbasel,max(mbasel,nveca)),source=0.0d0)
  allocate(tb(mbasel,nvecb),            source=0.0d0)
  allocate(va(nveca,mbasel),            source=0.0d0)
  allocate(vb(nvecb,mbasel),            source=0.0d0)
  allocate(w1(max(nelecs,nbas,mbasel)), source=0.0d0)
  allocate(w2(max(nelecs,nbas,mbasel),max(nelecs,nbas,mbasel)),source=0.0d0)
  allocate(taa(mbasel,max(mbasel,nveca)),source=0.0d0)

  allocate(u(nelecs,nelecs),            source=0.0d0)
  allocate(w(nelecs,nelecs),            source=0.0d0)
  allocate(wt(nelecs,nelecs),           source=0.0d0)
  allocate(ev(nelecs),                  source=0.0d0)
  allocate(diag(max(nelecs,nbas,mbasel)),   source=0.0d0)
  allocate(bdiag(max(nelecs,nbas,mbasel)),  source=0.0d0)
  allocate(cdiag(max(nelecs,nbas,mbasel)),  source=0.0d0)
  allocate(bsdiag(max(nelecs,nbas,mbasel)), source=0.0d0)
  allocate(csdiag(max(nelecs,nbas,mbasel)), source=0.0d0)
  allocate(sdiag(max(nelecs,nbas,mbasel)),  source=0.0d0)
  allocate(aaa(mbasel,max(mbasel,nveca)),   source=0.0d0)
  allocate(tt(mbasel,max(mbasel,nveca)),    source=0.0d0)
  allocate(aat(mbasel,max(mbasel,nveca)),   source=0.0d0)
  allocate(sm(mbasel,max(mbasel,nveca)),    source=0.0d0)
...
!$acc data create(va,vb,tb,ta,a,u,w,wt,ev,w1,w2,taa,sm,aaa,aat,tt,sdiag,diag,bsdiag,bdiag,csdiag,cdiag)
  ...
  allocate(workspace_d(len_work_dbl), source=0.0d0)
  allocate(workspace_i(len_work_int), source=0_8)
